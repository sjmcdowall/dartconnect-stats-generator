...
def find_data_files(data_dir: Path) -> Dict[str, Optional[Path]]:
    ...
    """
    files = {"by_leg": None, "cricket_leaderboard": None, "dart_501_leaderboard": None}
    ...
    for csv_file in csv_files:
        ...
        if (
            "by_leg" in name_lower or "by leg" in name_lower
        ) and "archived" not in name_lower:
            # Always use the newest by_leg file (in case there are multiple)
            if (
                files["by_leg"] is None
                or csv_file.stat().st_mtime > files["by_leg"].stat().st_mtime
            ):
    ...
    for file_type, file_path in files.items():
        if file_path:
            ...
            logger.info(
                f"📄 Found {file_type} file: {file_path.name} (modified: {readable_time})"
            )
    ...
    if total_urls > 0:
        ...
        if url_success_rate >= 0.8:
            ...
            assessment["recommendation"] = (
                "Excellent data quality - all features available"
            )
        ...
        assessment["recommendation"] = (
            "Consider using by_leg_export.csv for enhanced features"
        )
...
def process_data_with_fallback(
    data_files: Dict[str, Optional[Path]], config: Config
) -> Dict[str, Any]:
    """
    ...
    if data_files["by_leg"]:
        logger.info(
            f"🎯 Primary strategy: Processing {data_files['by_leg'].name} with URL enhancement"
        )
        try:
            ...
            if assessment["quality_score"] >= 75:
                logger.info(
                    f"✅ High quality results achieved (score: {assessment['quality_score']})"
                )
                ...
                logger.warning(
                    f"⚠️ URL processing had issues (score: {assessment['quality_score']})"
                )
    ...
    if data_files["cricket_leaderboard"] or data_files["dart_501_leaderboard"]:
        ...
        primary_file = (
            data_files["cricket_leaderboard"] or data_files["dart_501_leaderboard"]
        )
...
def save_json_results(results: Dict[str, Any], json_file: Path) -> None:
    ...
    if "raw_data" in json_results and hasattr(json_results["raw_data"], "to_dict"):
        json_results["raw_data"] = flatten_multiindex_columns(
            json_results["raw_data"]
        ).to_dict("records")

    if (
        "derived_metrics" in json_results
        and "rankings" in json_results["derived_metrics"]
    ):
    ...
    if (
        "statistics" in json_results
        and "player_statistics" in json_results["statistics"]
    ):
        player_stats = json_results["statistics"]["player_statistics"]
        if hasattr(player_stats, "to_dict"):
            json_results["statistics"]["player_statistics"] = (
                flatten_multiindex_columns(player_stats).to_dict("index")
            )
    ...
    try:
        ...
        logger = logging.getLogger(__name__)
        logger.warning(
            f"Full JSON serialization failed: {e}. Saving simplified version."
        )
        simplified_results = {
            ...
            "data_overview": {
                "total_records": (
                    len(json_results.get("raw_data", []))
                    if "raw_data" in json_results
                    else 0
                ),
...
def print_processing_summary(results: Dict[str, Any]) -> None:
    ...
    if raw_data is not None:
        ...
        if "game_name" in raw_data.columns:
            ...
            print(
                f"  Game Types: {', '.join([f'{game}: {count}' for game, count in game_types.head(3).items()])}"
            )
...
Examples:
  ...
  %(prog)s --config custom.yaml           # Use custom configuration
    ...
    parser.add_argument(
        "--data-dir", type=str, help="Alternative to input_path for directory scanning"
    )
    ...
    try:
        ...
        logger.info("🚀 Starting DartConnect Statistics Generator")
