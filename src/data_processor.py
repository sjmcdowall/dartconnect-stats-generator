...
PLACEHOLDER_NAMES = {"ghost", "placeholder", "test", "dummy"}
...
class DataProcessor:
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "last_name" in df.columns:
            ...
            if "player_name" in df.columns:
                ...
                df["player_name"] = df.apply(
                    lambda row: self._create_full_name(
                        row.get("last_name", ""), row.get("first_name_temp", "")
                    ),
                    axis=1,
                )
        ...
        if "_first_name_original" in df.columns:
            placeholder_mask = (
                df["_first_name_original"].str.lower().isin(PLACEHOLDER_NAMES)
            )
            if placeholder_mask.any():
                removed_count = placeholder_mask.sum()
                self.logger.info(
                    f"🚫 Filtered out {removed_count} placeholder player records (e.g., Ghost Player)"
                )
        ...
        if (
            "M/F" in df.columns
            and "player_name" in df.columns
            and "_first_name_original" in df.columns
        ):
    ...
    def _normalize_player_gender(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for player in df["player_name"].unique():
            ...
            has_missing = len(gender_values) < len(
                player_df
            )  # Some rows missing gender
                    ...
                    corrections.append(
                        {
                            ...
                            "records_updated": len(
                                player_df[player_df["M/F"] != latest_gender]
                            ),
                        }
                    )
        ...
        if corrections:
            self.logger.info(
                f"🔧 Normalized gender for {len(corrections)} player(s) with inconsistent values:"
            )
            for correction in corrections:
                old_vals_str = ", ".join(
                    [f"{k}({v})" for k, v in correction["old_values"].items()]
                )
                self.logger.info(
    ...
    def _infer_missing_gender(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            df.empty
            or "M/F" not in df.columns
            or "_first_name_original" not in df.columns
        ):
            return df
    ...
    def _calculate_derived_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "player_name" in df.columns and "score" in df.columns:
            ...
            qualified_players = player_summary[
                player_summary["games_played"] >= min_games
            ]

            # Rankings
            metrics["rankings"] = {
                "by_average": qualified_players.sort_values(
                    "average_score", ascending=False
                ),
                "by_total": qualified_players.sort_values(
                    "total_score", ascending=False
                ),
                "by_games": qualified_players.sort_values(
                    "games_played", ascending=False
                ),
            }
    ...
    def _calculate_time_statistics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        time_stats["games_per_month"] = {
            str(period): count for period, count in monthly_counts.items()
        }
        time_stats["games_per_week"] = {
            str(period): count for period, count in weekly_counts.items()
        }
        ...
        time_stats["recent_activity"] = {
            ...
            "active_players": (
                recent_games["player_name"].nunique()
                if "player_name" in df.columns
                else 0
            ),
        ...
        if (
            ...
            for player in df["player_name"].unique():
                ...
                if len(player_data) >= window:
                    player_data["rolling_avg"] = (
                        player_data["score"].rolling(window=window).mean()
                    )
                    ...
                    trends[player] = {
                        "trend_direction": (
                            "improving" if recent_avg > earlier_avg else "declining"
                        ),
    ...
    def _calculate_percentiles(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "score" in df.columns:
            score_percentiles = np.percentile(
                df["score"].dropna(), [25, 50, 75, 90, 95]
            )
    ...
    def _process_dartconnect_urls(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Enhanced data processing complete: "
    ...
    def _calculate_enhanced_qp_stats(
        self, df: pd.DataFrame, cricket_games: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def generate_team_statistics(
        ...
        if "Division" not in df.columns or "Team" not in df.columns:
            self.logger.warning(
                "Missing 'Division' or 'Team' columns, cannot generate team statistics."
            )
            return {}
        ...
        for division in divisions:
            ...
            division_teams = sorted(
                player_summary[player_summary["Division"] == division]["Team"]
                .dropna()
                .unique()
            )
            ...
            for team_name in division_teams:
                ...
                team_df["last_name"] = team_df["player_name"].apply(
                    lambda x: str(x).split()[-1] if " " in str(x) else str(x)
                )
                team_df = team_df.sort_values("last_name").drop(columns=["last_name"])

                # Add 'qualify' column
                team_df["qualify"] = team_df["games"].apply(
                    lambda x: max(0, games_to_qualify - x)
                )
                team_df["eligibility"] = team_df["games"].apply(
                    lambda x: "QUALIFIED" if x >= games_to_qualify else "INELIGIBLE"
                )

                teams_data.append(
                    {"name": team_name, "players": team_df.to_dict("records")}
                )
    ...
    def _calculate_game_outcomes_vectorized(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        game_groups = df.groupby(
            ["report_url", "Set #", "player_name", "game_name", "PF"]
        )
        ...
        game_outcomes = (
            legs_won.to_frame(name="legs_won")
            .join(legs_lost.to_frame(name="legs_lost"))
            .reset_index()
        )

        # Determine game outcome
        game_outcomes["game_win"] = (
            game_outcomes["legs_won"] > game_outcomes["legs_lost"]
        ).astype(int)
        game_outcomes["game_loss"] = (
            game_outcomes["legs_lost"] > game_outcomes["legs_won"]
        ).astype(int)

        return game_outcomes[
            ["player_name", "game_name", "PF", "game_win", "game_loss"]
        ]

    def _generate_player_stats_summary(
        self, df: pd.DataFrame, enhanced_data: Dict
    ) -> pd.DataFrame:
        ...
        win_loss_summary.columns = [
            f"{val}_{pf}_{col}".lower().replace(" ", "_").replace("-", "")
            for val, pf, col in win_loss_summary.columns
        ]

        # Rename columns to match the PDF generator's expectations
        column_renames = {
            "501_sido_s_game_win": "s01_w",
            "501_sido_s_game_loss": "s01_l",
            "cricket_s_game_win": "sc_w",
            "cricket_s_game_loss": "sc_l",
            "501_sido_d_game_win": "d01_w",
            "501_sido_d_game_loss": "d01_l",
            "cricket_d_game_win": "dc_w",
            "cricket_d_game_loss": "dc_l",
        }
        ...
        cricket_qps = {
            p: d.get("total_qp", 0)
            for p, d in enhanced_data.get("enhanced_statistics", {})
            .get("cricket_enhanced_qp", {})
            .items()
        }
        cricket_qp_series = pd.Series(cricket_qps, name="cricket_qp").rename_axis(
            "player_name"
        )

        # Merge all stats
        summary = (
            legs_played.join(games_played, how="left")
            .join(qp_501, how="left")
            .join(win_loss_summary, how="left")
            .join(cricket_qp_series, how="left")
            .fillna(0)
        )
        ...
        summary["win_pct"] = (
            (summary["total_w"] / total_games_played * 100)
            .fillna(0)
            .map("{:.2f}%".format)
        )

        summary["rating"] = (
            ((summary["total_w"] * 2) / summary["games"])
            + (summary["qps"] / summary["legs"])
        ).fillna(0)
        summary["rating"] = summary["rating"].map("{:.4f}".format)

        summary["qp_pct"] = (
            (summary["qps"] / summary["legs"] * 100).fillna(0).map("{:.2f}%".format)
        )
        ...
        for division in divisions:
            ...
            for team_name in division_teams:
                ...
                for _, full_name in player_list:
                    player_data = self._calculate_player_stats(
                        team_df, full_name, games_to_qualify, enhanced_data
                    )
                    players.append(player_data)

                teams_data.append({"name": team_name, "players": players})
    ...
    def _calculate_player_stats(
        ...
        team_df,
        player_name: str,
        games_to_qualify_threshold: int,
        ...
        eligibility = (
            "QUALIFIED" if games_played >= games_to_qualify_threshold else "INELIGIBLE"
        )
        ...
        win_pct = (
            f"{(total_wins / total_games * 100):.2f}%" if total_games > 0 else "0.00%"
        )
    ...
    def _calculate_game_specific_stats(self, player_df):
        ...
        if "report_url" not in player_df.columns or "Set #" not in player_df.columns:
            ...
            for game_type in player_df["game_name"].unique():
                for pf in player_df["PF"].unique():
                    game_df = player_df[
                        (player_df["game_name"] == game_type) & (player_df["PF"] == pf)
                    ]
                    if len(game_df) == 0:
                        continue
        ...
        for game_type in player_df["game_name"].unique():
            for pf in player_df["PF"].unique():
                ...
                for (match_url, set_num), set_data in game_df.groupby(
                    ["report_url", "Set #"]
                ):
