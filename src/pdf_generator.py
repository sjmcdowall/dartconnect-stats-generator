...
class PDFGenerator:
    ...
    def _create_individual_division_section(
        self, data: Dict[str, Any], division: str
    ) -> List:
        ...
        if not all_players:
            content.append(
                Paragraph(
                    f"No player data available for {division} division.",
                    self.styles["Normal"],
                )
            )
            return content
            ...
            for p in players:
                if category == "singles":
                    ...
                    if total > 0:
                        ...
                        player_list.append(
                            [p["name"], wins, losses, f"{win_pct:.2f}%", win_pct]
                        )
                    ...
                    if p["legs"] > 0:
                        ...
                        player_list.append(
                            [p["name"], p["qps"], f"{qp_pct:.2f}%", qp_pct]
                        )
        ...
        women_row = Table(
            [
                [
                    ...
                    self._create_category_table(
                        "All Events - Women",
                        women_stats["all_events"],
                        ["Name", "W", "L", "Win%"],
                    ),
                    self._create_category_table(
                        "Quality Points - Women",
                        women_stats["qp"],
                        ["Name", "Qp's", "QP%"],
                    ),
                ]
        ...
        men_row = Table(
            [
                [
                    ...
                    self._create_category_table(
                        "All Events - Men",
                        men_stats["all_events"],
                        ["Name", "W", "L", "Win%"],
                    ),
                    self._create_category_table(
                        "Quality Points - Men", men_stats["qp"], ["Name", "Qp's", "QP%"]
                    ),
                ]
            ],
            colWidths=[2.4 * inch, 2.4 * inch, 2.4 * inch],
        ...
        content.extend(
            self._create_ratings_and_qp_register_section(
                women_stats["ratings"], men_stats["ratings"], division
            )
        )
    ...
    def _create_ratings_and_qp_register_section(
        self, women_ratings: List, men_ratings: List, division: str
    ) -> List:
        ...
        ratings_row = Table(
            [[women_table, special_qp, men_table]],
            colWidths=[2.2 * inch, 3.0 * inch, 2.2 * inch],
        )
        ratings_row.setStyle(
            TableStyle(
                [
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
                    (
                        ...
                        (1, 0),
                        "CENTER",
                    ),  # Center the special QP register
                ]
            )
        )
    ...
    def _extract_special_achievements(self, division: str) -> Dict:
        ...
        if (
            isinstance(self.enhanced_data, dict)
            and "cricket_qp_data" in self.enhanced_data
        ):
                    ...
                    if player_name in div_df["player_name"].values:
                        for turn in player_data.get("turn_data", []):
                            ...
                            if turn.get("bulls", 0) >= 6:
                                achievements["six_bulls"][player_name] = (
                                    achievements["six_bulls"].get(player_name, 0) + 1
                                )
                            # Check for 9+ marks (9 hits)
                            if turn.get("marks", 0) >= 9:
                                achievements["nine_hits"][player_name] = (
                                    achievements["nine_hits"].get(player_name, 0) + 1
                                )
    ...
    def _calculate_week_number(self, data: Dict[str, Any]) -> int:
        ...
        if "game_date" in df.columns:
            ...
            if len(unique_dates) > 0:
                ...
                week_number = (
                    days_diff // 7
                ) + 1  # +1 because first week is Week 1, not Week 0
    ...
    def _create_league_summary(self, data: Dict[str, Any]) -> List:
        ...
        if date_range.get("start") and date_range.get("end"):
            start_date = datetime.fromisoformat(date_range["start"]).strftime(
                "%B %d, %Y"
            )
    ...
    def _create_player_rankings(self, data: Dict[str, Any]) -> List:
        ...
        if "by_average" in rankings:
            content.append(
                Paragraph("Top 10 Players by Average Score", self.styles["Heading3"])
            )
            ...
            table = Table(
                ranking_data,
                colWidths=[0.8 * inch, 2.5 * inch, 1 * inch, 1 * inch, 1.2 * inch],
            )
            table.setStyle(
                TableStyle(
                    [
    ...
    def _create_team_performance_section(
        self, data: Dict[str, Any], division_title: str
    ) -> List:
        ...
        for team in teams:
            ...
            for i, player in enumerate(team["players"]):
                ...
                if player["eligibility"] == "QUALIFIED":
                    style.append(
                        ("TEXTCOLOR", (4, player_row), (4, player_row), colors.green)
                    )
                else:
                    style.append(
                        ("TEXTCOLOR", (4, player_row), (4, player_row), colors.red)
                    )
    ...
    def _create_individual_performance(self, data: Dict[str, Any]) -> List:
        ...
        content = []
        content.append(
            Paragraph("Individual Player Performance", self.styles["SectionHeader"])
        )
        ...
        if player_stats is not None:
            ...
            if hasattr(player_stats.columns, "levels"):
                ...
                player_stats.columns = [
                    "_".join(col).strip() for col in player_stats.columns.values
                ]

            # Create a comprehensive player table
            player_data = [
                ["Player", "Games", "Avg Score", "Best Score", "Worst Score"]
            ]
            ...
            for player in player_stats.index[:15]:  # Top 15 players
                ...
                games = self._get_stat_value(
                    player_stats, player, ["score_count", "games", "count"]
                )
                avg_score = self._get_stat_value(
                    player_stats, player, ["score_mean", "average", "mean"]
                )
                max_score = self._get_stat_value(
                    player_stats, player, ["score_max", "max"]
                )
                min_score = self._get_stat_value(
                    player_stats, player, ["score_min", "min"]
                )
            ...
            table = Table(
                player_data,
                colWidths=[2.5 * inch, 1 * inch, 1.2 * inch, 1.2 * inch, 1.2 * inch],
            )
            table.setStyle(
                TableStyle(
                    [
    ...
    def _create_performance_trends(self, data: Dict[str, Any]) -> List:
        ...
        if trends:
            # Create trends table
            trend_data = [
                ["Player", "Trend", "Recent Avg", "Earlier Avg", "Improvement"]
            ]
            ...
            table = Table(
                trend_data,
                colWidths=[2 * inch, 1.2 * inch, 1.2 * inch, 1.2 * inch, 1.2 * inch],
            )
            table.setStyle(
                TableStyle(
                    [
    ...
    def _create_detailed_statistics(self, data: Dict[str, Any]) -> List:
        ...
        if "recent_activity" in time_stats:
            ...
            content.append(
                Paragraph("Recent Activity (Last 30 Days)", self.styles["Heading3"])
            )
    ...
    def _create_overall_division_section(
        self, data: Dict[str, Any], division_title: str
    ) -> List:
        ...
        if teams:
            ...
            content.append(
                Paragraph(
                    f"No team data available for {division_title}.",
                    self.styles["Normal"],
                )
            )
    ...
    def _create_division_table(self, teams: List[Dict], division_name: str) -> List:
        ...
        col_widths = [
            ...
            0.28 * inch,
            0.28 * inch,  # S01 W/L (slightly wider)
            0.28 * inch,
            0.28 * inch,  # SC W/L
            0.28 * inch,
            0.28 * inch,  # D01 W/L
    ...
    def _create_special_achievements_section(
        self, data: Dict[str, Any], division: str
    ) -> List:
